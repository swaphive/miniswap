...
async function swapHiveAccDetails () {
    try
    {
        let swapHiveData = await ssc.findOne('tokens', 'balances', {'account': 'miniswap', 'symbol': 'SWAP.HIVE'});
...
async function hiveAccDetails () {
    try 
    {
        let hiveData = await hive.api.callAsync('condenser_api.get_accounts', [['miniswap']]);
